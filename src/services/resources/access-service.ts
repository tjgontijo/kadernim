...
export async function grantAccessService(
  ...
  const existingAccess = await prisma.userResourceAccess.findUnique({
    where: {
      userId_resourceId: {
        userId,
        resourceId,
  ...
  const access = await prisma.userResourceAccess.create({
    data: {
      userId,
      resourceId,
      source: 'manual',
...
export async function revokeAccessService(
  ...
  await prisma.userResourceAccess.delete({
    where: { id: accessId },
  })
}
...
export async function getResourceAccessService(
  ...
  const access = await prisma.userResourceAccess.findMany({
    where: { resourceId },
    include: {
      user: {
        select: {
