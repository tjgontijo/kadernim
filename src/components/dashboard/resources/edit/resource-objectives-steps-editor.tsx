...
interface ResourceObjectivesStepsEditorProps {
  resourceId: string
  sections?: Array<'objectives' | 'steps'>
}

export function ResourceObjectivesStepsEditor({
  resourceId,
  sections,
}: ResourceObjectivesStepsEditorProps) {
  const queryClient = useQueryClient()
  const visibleSections = new Set(sections ?? ['objectives', 'steps'])
  const showObjectives = visibleSections.has('objectives')
  const showSteps = visibleSections.has('steps')

  const { data: initialObjectives = [], isLoading: isLoadingObjectives } = useQuery({
    queryKey: ['admin-resource-objectives', resourceId],
    queryFn: () => fetchResourceObjectives(resourceId),
    enabled: showObjectives,
  })

  const { data: initialSteps = [], isLoading: isLoadingSteps } = useQuery({
    queryKey: ['admin-resource-steps', resourceId],
    queryFn: () => fetchResourceSteps(resourceId),
    enabled: showSteps,
  })
  ...
  useEffect(() => {
    ...
    const steps = showSteps ? initialSteps : (form.getValues('steps') ?? [])

    form.reset({
      objectives: objectives.length > 0 ? objectives : [{ text: '', order: 1 }],
      steps:
        steps.length > 0
          ? steps
          : [{ type: 'DISCUSSION', title: '', content: '', order: 1 }],
    })
  }, [form, initialObjectives, initialSteps, showObjectives, showSteps])

  const applyObjectives = (objectives: PedagogicalContent['objectives'], message: string) => {
    form.setValue('objectives', objectives)
    queryClient.invalidateQueries({ queryKey: ['admin-resource-objectives', resourceId] })
    queryClient.invalidateQueries({ queryKey: ['resource-detail', resourceId] })
    toast.success(message)
  }

  const applySteps = (steps: PedagogicalContent['steps'], message: string) => {
    form.setValue('steps', steps)
    queryClient.invalidateQueries({ queryKey: ['admin-resource-steps', resourceId] })
    queryClient.invalidateQueries({ queryKey: ['resource-detail', resourceId] })
    toast.success(message)
  }
  ...
  const saveObjectivesMutation = useMutation({
    mutationFn: (objectives: PedagogicalContent['objectives']) =>
      updateResourceObjectives(resourceId, objectives),
    onSuccess: (objectives) => applyObjectives(objectives, 'Objetivos atualizados com sucesso!'),
    onError: (error) => {
  ...
  const saveStepsMutation = useMutation({
    mutationFn: (steps: PedagogicalContent['steps']) => updateResourceSteps(resourceId, steps),
    onSuccess: (steps) => applySteps(steps, 'Passo a passo atualizado com sucesso!'),
    onError: (error) => {
  ...
  if ((showObjectives && isLoadingObjectives) || (showSteps && isLoadingSteps)) {
    return (
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  onClick={() => appendObjective({ text: '', order: objectiveFields.length + 1 })}
                >
