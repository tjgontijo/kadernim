...
import type { CampaignInput } from '@/lib/campaigns/schemas'
...
export class CampaignService {
  ...
  static async getAnalytics(period = '30d') {
    ...
    const campaignRanking = campaigns.map((campaign) => ({
      ...campaign,
      ctr: campaign.totalSent > 0 ? ((campaign.totalClicked / campaign.totalSent) * 100).toFixed(2) : '0.00',
    }))
    ...
    return {
      kpis: {
        ...
        topCampaign: topCampaign
          ? {
              ...topCampaign,
              ctr: topCampaign.totalSent > 0
                ? ((topCampaign.totalClicked / topCampaign.totalSent) * 100).toFixed(2)
                : '0.00',
            }
          : null,
        totalUsersWithPush,
      ...
      daily: dailyData.map((daily) => ({
        date: new Date(daily.date).toISOString().split('T')[0],
        sent: Number(daily.sent),
        clicked: Number(daily.clicked),
      })),
