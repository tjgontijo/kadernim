...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const existing = await PushTemplateService.getById(id)
    ...
    if (parsed.data.slug && parsed.data.slug !== existing.slug) {
      const slugExists = await PushTemplateService.getBySlug(parsed.data.slug)
    ...
    const template = await PushTemplateService.update(id, parsed.data)
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    await PushTemplateService.delete(id)
